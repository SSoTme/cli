...
if (-not (Test-Path (Join-Path $RootDir "Effortless.Cli.sln"))) {
    Write-Error "Cannot find Effortless.Cli.sln at expected location: $RootDir"
    Write-Error "Please run this script from installers/windows/Scripts/"
    exit 1
}

$SourceDir = Join-Path $RootDir "src\Effortless.Cli"
...
Set-Content "$SourceDir/Effortless.Cli.csproj" $CSPROJ_CONTENT_new -NoNewline
Write-Host "Updated version of $SourceDir/Effortless.Cli.csproj to $csprojVersion"
