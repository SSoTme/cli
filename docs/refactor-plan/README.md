...
| D3 | **Authentication while the service endpoint is not enabled.** | **Superseded by step 13 (2026-09-05):** `login`, `projectLogin`, `plan`, `logout` call the published `effortless-auth` tool through the catalog. The service is a preview that always succeeds; tool execution and `buildOnTrigger` remain unauthenticated. | `CliOptions.authenticate/projectLogin/subscription`, `HttpEndpoints.bridge-auth`, `TestCases auth-*` |
